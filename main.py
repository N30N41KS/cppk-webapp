...
# --- ЗАГРУЗКА НАСТРОЕК ИЗ .env (Безопасность) ---
load_dotenv()
...
def init_db():
    ...
    cursor = conn.cursor()
    # Таблица пользователей (для рангов)
    cursor.execute(
        '''CREATE TABLE IF NOT EXISTS users (user_id INTEGER PRIMARY KEY, reports_count INTEGER DEFAULT 0)''')
    # Таблица общей статистики диспетчерских
    cursor.execute('''CREATE TABLE IF NOT EXISTS stats (
                        id INTEGER PRIMARY KEY CHECK (id = 1),
                        mcd_total INTEGER DEFAULT 0,
                        mcd_resolved INTEGER DEFAULT 0,
                        metro_total INTEGER DEFAULT 0,
                        metro_resolved INTEGER DEFAULT 0)''')
    # Добавляем стартовую строчку, если база новая
    cursor.execute(
        '''INSERT OR IGNORE INTO stats (id, mcd_total, mcd_resolved, metro_total, metro_resolved) VALUES (1, 0, 0, 0, 0)''')
    conn.commit()
    conn.close()
...
def update_user_stat(user_id):
    conn = sqlite3.connect('database.db')
    cursor = conn.cursor()
    cursor.execute('''INSERT INTO users (user_id, reports_count) VALUES (?, 1) 
                      ON CONFLICT(user_id) DO UPDATE SET reports_count = reports_count + 1''', (user_id,))
    conn.commit()
    conn.close()
...
def get_admin_stats():
    conn = sqlite3.connect('database.db')
    cursor = conn.cursor()
    cursor.execute('''SELECT mcd_total, mcd_resolved, metro_total, metro_resolved FROM stats WHERE id = 1''')
    row = cursor.fetchone()
    conn.close()
...
async def cmd_about(message: types.Message):
    text = (
        ...
        "🧠 <i>Под капотом: интеграция с нейросетью GigaChat, Telegram WebApp и база данных SQLite.</i>\n\n"
...
async def cmd_dashboard(message: types.Message):
    mcd_tot, mcd_res, metro_tot, metro_res = get_admin_stats()
    text = (
        "📈 <b>ОПЕРАТИВНАЯ СВОДКА</b>\n\n"
        "🚆 <b>ЦППК (МЦД):</b>\n"
        f"Поступило заявок: {mcd_tot}\n"
        f"Успешно закрыто: {mcd_res}\n\n"
        "🚇 <b>Дептранс (Метро/МЦК):</b>\n"
        f"Поступило заявок: {metro_tot}\n"
        f"Успешно закрыто: {metro_res}\n\n"
        "<i>*Данные надежно сохранены в базе SQLite</i>"
    )
    await message.answer(text, parse_mode="HTML")
...
async def handle_webapp(message: types.Message, state: FSMContext):
    ...
    # Считаем статистику в Базе Данных!
    update_user_stat(message.from_user.id)
    if category == 'metro':
        update_admin_stat('metro_total')
    else:
        update_admin_stat('mcd_total')
...
async def handle_adm(callback: CallbackQuery):
    ...
    if action == "work":
        ...
        # Обновляем счетчик закрытых заявок в Базе Данных!
        if admin_chat_id == ADMIN_CHAT_ID_METRO:
            update_admin_stat('metro_resolved')
        elif admin_chat_id == ADMIN_CHAT_ID_CPPK:
            update_admin_stat('mcd_resolved')
